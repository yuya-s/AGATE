...
class BADataset():
    def __init__(self, path, L, is_train, is_valid, is_test):
        ...
        for n in range(len(self.adjacency)):
            ...
            indic = np.array(list(graph.edges)) # (nnz/2, 2) (：[0, 1][1, 0])
            ...
            # train_ones
            indic_train_ones = np.delete(indic, obj=idx_valid, axis=0)

            # train_zeros
            indic_train_zeros = set()
            while len(indic_train_zeros) < len(indic_train_ones):
                i = int(indic_train_ones[random.sample(list(range(indic_train_ones.shape[0])), 1), 0])
                j = int(indic[random.sample(list(range(indic.shape[0])), 1), 1])
                while i == j:
                    j = int(indic[random.sample(list(range(indic.shape[0])), 1), 1])
                if not ((frozenset([i, j]) in indic_frozenset) or (frozenset([i, j]) in indic_valid_zeros_frozenset)):
                    indic_train_zeros.add(frozenset([i, j]))
            indic_train_zeros = np.array([list(edge) for edge in indic_train_zeros])
            ...
            # indic_test_ones = np.array(list(graph.edges))  # (nnz/2, 2) (：[0, 1][1, 0])

            # data_arrays (indic_train_ones, indic_train_zeros, indic_valid_ones, indic_valid_zeros, indic_test)
            data_arrays_link = [indic_train_ones, indic_train_zeros, indic_valid_ones, indic_valid_zeros, indic_test]
            #np.savez('data_arrays_link', indic_train, indic_valid_ones, indic_valid_zeros, indic_test)

            # distance matrix
            dists = precompute_dist_data(indic_train_ones, all_node_num + n_expanded, approximate=-1)
            ...
            #ind_train_A[del_idx_ls[0], del_idx_ls[1]] = 0 # val
            #ind_train_A[del_idx_ls[1], del_idx_ls[0]] = 0 # val
            ...
            # nodes_keep
            nodes_keep = np.array(sorted(set(sum(indic_train_ones.tolist(), []))))
            ...
            # 
            indic_train_frozenset = set([frozenset({i, j}) for [i, j] in indic_train_ones])
    ...
    def __getitem__(self, index):
        ...
        """
        print(type(sample_idx), type(data_arrays_link), type(dists), type(ind_train_A), type(ind_train_X), type(nodes_keep), type(A), type(X), type(label), type(mask))
        <class 'int'> <class 'list'> <class 'numpy.ndarray'> <class 'numpy.ndarray'> <class 'numpy.ndarray'> <class 'numpy.ndarray'> <class 'numpy.ndarray'> <class 'numpy.ndarray'> <class 'numpy.ndarray'> <class 'numpy.ndarray'><class 'int'> <class 'list'> <class 'numpy.ndarray'> <class 'numpy.ndarray'> <class 'numpy.ndarray'> <class 'numpy.ndarray'> <class 'numpy.ndarray'> <class 'numpy.ndarray'> <class 'numpy.ndarray'> <class 'numpy.ndarray'>

        print(sample_idx, len(data_arrays_link), dists.shape, ind_train_A.shape, ind_train_X.shape, nodes_keep.shape, A.shape, X.shape, label.shape, mask.shape)
        46 4 (3976, 3976) (3859, 3859) (3859, 35) (439,) (3976, 3976) (3976, 35) (3976, 3976) (3976, 3976)
        
        print(data_arrays_link[0].shape)
        print(data_arrays_link[1].shape)
        print(data_arrays_link[2].shape)
        print(data_arrays_link[3].shape)
        print(data_arrays_link[4].shape)
        (2817, 2)
        (2817, 2)
        (313, 2)
        (313, 2)
