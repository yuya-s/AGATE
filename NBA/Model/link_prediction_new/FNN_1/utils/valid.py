...
def valid(epoch, dataloader, net, criterion, opt):
    valid_loss = 0
    net.train()
    for i, (sample_idx, data_arrays_link, dists, ind_train_A, ind_train_X, nodes_keep, A, X, label, mask) in enumerate(dataloader, 0):
        ...
        valid_loss_ = 0
        for j in range(indic_valid_ones.shape[0]):
            X_p_0 = X[indic_valid_ones[j, 0]]
            X_p_1 = X[indic_valid_ones[j, 1]]
            X_n_0 = X[indic_valid_zeros[j, 0]]
            X_n_1 = X[indic_valid_zeros[j, 1]]
            X_batch = torch.stack([torch.stack([X_p_0, X_p_1]), torch.stack([X_n_0, X_n_1])])
            output = net(X_batch)
            loss = criterion(output, torch.DoubleTensor([[1], [0]]))
            valid_loss_ += loss.item()
        valid_loss += valid_loss_ / (indic_valid_ones.shape[0])
