...
class Propagator(nn.Module):
    ...
    def bmm_sparse(self, A, state):
        ...
        """
        a_batch = []
        for batch in range(A.shape[0]): # batch×
            a_ts = []
            for time_step in range(self.L): # time_step×
                A_sparse = coo_numpy2sparse_pytorch(A[batch], adj_shape) # torch.sparse (COO)
                a_ts.append(torch.sparse.mm(A_sparse, state[batch, :, time_step, :])) # ×
            a_batch.append(torch.stack(a_ts, 1)) # axis=1 (time_step)stack
        a = torch.stack(a_batch, 0) # axis=0 (batch)stack
        return a
