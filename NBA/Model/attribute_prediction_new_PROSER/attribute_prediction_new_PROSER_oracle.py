...
from setting_param import Model_attribute_prediction_new_PROSER_oracle_InputDir as InputDir
from setting_param import Model_attribute_prediction_new_PROSER_oracle_OutputDir as OutputDir
...
from setting_param import attribute_prediction_new_PROSER_target_idx as target_idx
...
for th in th_ls:
    ...
    for ts in range(L, EXIST_TABLE.shape[1] - L):
        ...
        explore_idx = np.array([j for j in range(node_attribute_new.shape[0])])[labels[ts - L][:node_attribute_new.shape[0]] >= th]
        while True:
...
df.to_csv(OutputDir + '/result_oracle_train_valid.csv')
...
plt.ylabel('Gain')
plt.title('mean')
plt.savefig(OutputDir + '/result_oracle_train_valid_mean.pdf')
...
plt.ylabel('Gain')
plt.title('min')
plt.savefig(OutputDir + '/result_oracle_train_valid_min.pdf')
...
plt.ylabel('Gain')
plt.title('median')
plt.savefig(OutputDir + '/result_oracle_train_valid_median.pdf')
...
plt.ylabel('Gain')
plt.title('max')
plt.savefig(OutputDir + '/result_oracle_train_valid_max.pdf')
