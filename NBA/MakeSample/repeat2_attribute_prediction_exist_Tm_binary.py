...
from setting_param import MakeSample_repeat2_attribute_prediction_exist_Tm_binary_InputDir as InputDir
from setting_param import MakeSample_repeat2_attribute_prediction_exist_Tm_binary_utilize_existing_attribute_OutputDir as OutputDir_0
from setting_param import MakeSample_repeat2_attribute_prediction_exist_Tm_binary_utilize_lost_OutputDir as OutputDir_1
from setting_param import MakeSample_repeat2_attribute_prediction_exist_Tm_binary_utilize_new_attribute_link_OutputDir as OutputDir_2
from setting_param import MakeSample_repeat2_attribute_prediction_exist_Tm_binary_utilize_disappeared_OutputDir as OutputDir_4
from setting_param import MakeSample_repeat2_attribute_prediction_exist_Tm_binary_utilize_appeared_OutputDir as OutputDir_8
from setting_param import MakeSample_repeat2_attribute_prediction_exist_Tm_binary_utilize_all_OutputDir as OutputDir_15
...
from repeat2_utils.get_predicted_existing_attribute import get_predicted_existing_attribute
from setting_param import best_methods_attribute_existing_OutputDir2 as best_methods_attribute_existing_OutputDir
...
for c_idx in range(16):
    ...
    for ts in range(L, EXIST_TABLE.shape[1] - L):
        ...
        tm_prev = NodeAttribute(ts_train[-1])[:, Tm_idx].argmax(axis=1)[:, np.newaxis] * mask[:all_node_num]
        tm_next = NodeAttribute(ts_test)[:, Tm_idx].argmax(axis=1)[:, np.newaxis] * mask[:all_node_num]
        label_ = (tm_prev != tm_next).astype(np.double)
        label = np.zeros((all_node_num + n_expanded, 1))
        label[:all_node_num] = label_
