...
class BADataset():
    ...
    def __getitem__(self, index):
        ...
        # test_ones
        indic_test_ones = torch.Tensor(mmread(self.label_paths[index]).toarray()).to_sparse()._indices().numpy().transpose(1, 0)  # (nnz, 2) (：[0, 1][1, 0])
        graph = nx.Graph()
        ...
        # mask ー
        indic_mask = torch.Tensor(mmread(self.mask_paths[index]).toarray()).to_sparse()._indices().numpy().transpose(1, 0)  # (nnz, 2) (：[0, 1][1, 0])
        graph = nx.Graph()
        graph.add_edges_from(indic_mask.tolist())
        indic_mask = np.array(sorted(graph.edges))  # (nnz/2, 2) (：[0, 1][1, 0])
        indic_mask = np.array([sorted(edge, reverse=True) for edge in indic_mask])  # (new_node, existing_node)
        mask = indic_mask
